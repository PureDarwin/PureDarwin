...
static void
spin_for_timeout(uint32_t seconds)
{
...
T_DECL(perf_csw, "context switch performance", T_META_TAG_PERF, T_META_CHECK_LEAKS(false), T_META_ASROOT(true), T_META_TAG_VM_NOT_ELIGIBLE, T_META_ENABLED(TARGET_CPU_ARM64))
{
	...
	for (uint32_t thread_id = 0; thread_id < n_spinners; thread_id++) {
		threads[thread_id].thread = make_thread(thread_id, THR_SPINNER_PRI,
		    true, &spin_thread);
	}

	s = dt_stat_time_create("context switch time");

	make_thread(n_spinners, THR_MANAGER_PRI, true, &thread);
	threads[n_spinners].measure_thread = true;
	make_thread(n_spinners + 1, THR_MANAGER_PRI, true, &thread);
