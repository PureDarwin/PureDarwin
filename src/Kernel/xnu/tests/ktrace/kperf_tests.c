...
#define PERF_DISPLABEL  KDBG_EVENTID(DBG_PERF, 1, 23)
#define PERF_DISPSAMPLE KDBG_EVENTID(DBG_PERF, 1, 10)
	...
	T_META_TAG_VM_NOT_ELIGIBLE)
{
	...
	ktrace_events_single(s, PERF_TMR_FIRE, ^(struct trace_point *tp) {
		...
		T_QUIET;
		T_ASSERT_LT((int)tp->cpuid, ncpus,
...
{
	(void)kperf_reset();
	(void)sysctlbyname("kperf.debug_level", NULL, NULL, &(int){ 0 },
	    sizeof(int));
}
...
#define UNCOMMON_EVENTID 0xfeedfac0
#define LABEL "this_is_a_test_label_that_is_very_long_and_will_be_truncated" \
		"_but_needs_some_more_length_for_good_measure"
	...
	T_META_TAG_VM_PREFERRED)
{
	...
	/*
	 * Expect to see the label that was sampled by kperf.
	 */
	__block uint64_t nlabels = 0;
	char obs_label[MAX_LABEL_LEN + 1] = {};
	char *obs_label_b = obs_label;
	char exp_label[MAX_LABEL_LEN + 1] = {};
	strlcpy(exp_label, LABEL, sizeof(exp_label));
	char *exp_label_b = exp_label;

	ktrace_events_single(sess, PERF_DISPLABEL, ^(struct trace_point *tp) {
		nlabels++;
		size_t argsize = ktrace_is_kernel_64_bit(sess) ? 8 : 4;
		strncat(obs_label_b, (char *)&tp->arg1, argsize);
		strncat(obs_label_b, (char *)&tp->arg2, argsize);
		strncat(obs_label_b, (char *)&tp->arg3, argsize);
		strncat(obs_label_b, (char *)&tp->arg4, argsize);
		if (tp->debugid & DBG_FUNC_END) {
			ktrace_end(sess, 1);
		}
	});
	ktrace_events_single(sess, PERF_DISPSAMPLE, ^(struct trace_point *tp) {
		if (tp->debugid & DBG_FUNC_END) {
			T_LOG("ending sample event returned %d (%s)",
			    (int)tp->arg1, strerror((int)tp->arg1));
		}
	});
	ktrace_events_single(sess, UNCOMMON_EVENTID,
	    ^(__unused struct trace_point *tp) {
		T_LOG("saw triggering event");
	});
	ktrace_set_completion_handler(sess, ^{
		T_EXPECT_GT(nlabels, 0ULL, "saw %" PRIu64
		    " dispatch queue label events", nlabels);
		T_EXPECT_EQ_STR(obs_label_b, exp_label_b, "label string is correct");
		T_END;
	});

	/*
	 * Set up kperf to log a dispatch queue label when an uncommon event ID
	 * is traced.
	 */
	(void)sysctlbyname("kperf.debug_level", NULL, NULL, &(int){ 2 }, sizeof(int));
	(void)kperf_action_count_set(1);
	T_ATEND(reset_kperf);
	T_QUIET;
	T_ASSERT_POSIX_SUCCESS(kperf_action_samplers_set(1,
	    KPERF_SAMPLER_TH_DISPATCH), NULL);
	(void)kperf_kdebug_action_set(1);
	kperf_kdebug_filter_t kdfilt = kperf_kdebug_filter_create();
	T_QUIET;
	T_ASSERT_NOTNULL(kdfilt, "create kdebug filter for kperf");
	T_QUIET;
	T_ASSERT_POSIX_SUCCESS(
	    kperf_kdebug_filter_add_debugid(kdfilt, UNCOMMON_EVENTID),
	    "add debugid to filter on");
	T_QUIET;
	T_ASSERT_POSIX_SUCCESS(
	    kperf_kdebug_filter_set(kdfilt), "set filter for kperf");
	T_ASSERT_POSIX_SUCCESS(kperf_sample_set(1), "start kperf sampling");
	...
	/*
	 * Create the queue and emit the sampling event from it.
	 */
	dispatch_queue_t testq = dispatch_queue_create(LABEL, 0);
	T_ASSERT_NOTNULL(testq, "created queue with label: " LABEL);
	dispatch_async(testq, ^{
		kdebug_trace(UNCOMMON_EVENTID, 0, 0, 0, 0);
		sleep(10);
	});
...
T_HELPER_DECL(kperf_blessed_helper,
    "drop priviliges and attempt to configure kperf")
{
	drop_priv();
	(void)kperf_action_count_set(1);
	errno = 0;
	int ret = kperf_lazy_wait_action_set(1);
	int error = errno;
	exit(ret != 0 ? (error == 0) ? EINVAL : error : 0);
}

static pid_t
spawn_bless_helper(void)
{
	char test_path[MAXPATHLEN] = { 0 };
	int ret = proc_pidpath(getpid(), test_path, MAXPATHLEN);
	T_QUIET; T_ASSERT_POSIX_SUCCESS(ret, "query path for current process");
	T_LOG("spawning helper tool at %s", test_path);
	char *child_argv[] = { test_path, "-n", "kperf_blessed_helper", NULL };
	pid_t child_pid = -1;
	int error = dt_launch_tool(&child_pid, child_argv, true, NULL, NULL);
	T_ASSERT_POSIX_ZERO(error, "spawned child with pid %d", child_pid);
	T_QUIET; T_ASSERT_GT(child_pid, 0, "child pid is valid");
	return child_pid;
}

static int
test_child_process(pid_t child_pid)
{
	int ret = kill(child_pid, SIGCONT);
	T_ASSERT_POSIX_SUCCESS(ret, "continuing child process");
	int status = 0;
	ret = waitpid(child_pid, &status, 0);
	T_ASSERT_POSIX_SUCCESS(ret, "waited on child process");
	return status;
}
	...
	T_META_TAG_VM_PREFERRED)
{
	...
	T_SETUPEND;
	int status = test_child_process(child_pid);
	int exit_status = WEXITSTATUS(status);
	T_EXPECT_NE(exit_status, 0, "child failed to access ktrace with %d (%s)",
	...
	T_META_TAG_VM_PREFERRED)
{
	...
	pid_t child_pid = spawn_bless_helper();
	T_SETUPEND;
	int ret = kperf_bless_set(child_pid);
	T_ASSERT_POSIX_SUCCESS(ret, "blessed child with pid %d", child_pid);
	int status = test_child_process(child_pid);
	int exit_status = WEXITSTATUS(status);
	T_EXPECT_EQ(exit_status, 0, "child status was %d (%s)", exit_status,
	    exit_status == 0 ? "ok" : strerror(exit_status));
