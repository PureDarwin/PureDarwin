...
static OSObject *
pd_ioreg_unserializable_string(const OSMetaClassBase *obj)
{
	char desc[128];
	const OSMetaClass *meta;

	meta = obj ? obj->getMetaClass() : NULL;
	snprintf(desc, sizeof(desc), "%s is not serializable",
	    meta ? meta->getClassName() : "unknown class");

	return OSString::withCString(desc);
}

static bool
pd_ioreg_object_serializes(const OSMetaClassBase *obj)
{
	bool ok;
	OSSerialize *s;
...
static OSObject *
pd_ioreg_sanitize_object(const OSMetaClassBase *obj, unsigned int depth)
{
	OSArray *array;
	OSArray *copyArray;
	OSDictionary *dict;
	...
	if (depth > 16) {
		return pd_ioreg_unserializable_string(obj);
	}

	if ((dict = OSDynamicCast(OSDictionary, obj))) {
		copyDict = OSDictionary::withCapacity(dict->getCount() ? dict->getCount() : 1);
		if (!copyDict) {
			return NULL;
		}
		...
		while ((key = iter->getNextObject())) {
			const OSSymbol *sym;
			OSObject *sanitized;

			sym = OSDynamicCast(OSSymbol, key);
			if (!sym) {
				continue;
			}

			sanitized = pd_ioreg_sanitize_object(dict->getObject(sym), depth + 1);
			if (!sanitized) {
				continue;
			}

			copyDict->setObject(sym, sanitized);
			sanitized->release();
		}
		iter->release();
	...
	if ((array = OSDynamicCast(OSArray, obj))) {
		copyArray = OSArray::withCapacity(array->getCount() ? array->getCount() : 1);
		if (!copyArray) {
			return NULL;
		}
		...
		for (unsigned int i = 0; i < array->getCount(); i++) {
			...
			sanitized = pd_ioreg_sanitize_object(array->getObject(i), depth + 1);
			if (!sanitized) {
				continue;
			}

			copyArray->setObject(sanitized);
			sanitized->release();
		}
	...
	if (pd_ioreg_object_serializes(obj)) {
		osObj = OSDynamicCast(OSObject, obj);
		if (osObj) {
			osObj->retain();
			return const_cast<OSObject *>(osObj);
		}
	}
	...
	mach_msg_type_number_t *propertiesCnt )
{
	...
	sanitized = pd_ioreg_sanitize_object(dict, 0);
	dict->release();
	if (!sanitized) {
		return kIOReturnNoMemory;
	}
