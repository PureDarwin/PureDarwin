...
#define DEBUG(x, ...) do { \
    if (debug_enabled()) kprintf("IOIntelFramebuffer: " x, ##__VA_ARGS__); \
} while (0)

#define super IOFramebuffer
OSDefineMetaClassAndStructors(IOIntelFramebuffer, IOFramebuffer);
...
uint16_t
IOIntelFramebuffer::findPchDeviceId(void)
{
...
bool
IOIntelFramebuffer::mapScanoutGtt(void)
{
...
bool
IOIntelFramebuffer::lilBringup(void)
{
...
IOService *
IOIntelFramebuffer::probe(IOService *provider, SInt32 *score)
{
    DEBUG("probe %p\n", provider);
...
bool
IOIntelFramebuffer::start(IOService *provider)
{
    DEBUG("start %p\n", provider);
...
void
IOIntelFramebuffer::stop(IOService *provider)
{
    DEBUG("stop %p\n", provider);
    if (fFbBacking) {
        fFbBacking->complete();
...
IOReturn IOIntelFramebuffer::enableController() { return kIOReturnSuccess; }

const char *IOIntelFramebuffer::getPixelFormats() { return IO32BitDirectPixels; }

IOReturn
IOIntelFramebuffer::getCurrentDisplayMode(IODisplayModeID *displayMode, IOIndex *depth)
{
...
IOReturn IOIntelFramebuffer::setDisplayMode(IODisplayModeID, IOIndex)
{
    return kIOReturnSuccess;
}

IODeviceMemory *
IOIntelFramebuffer::getApertureRange(IOPixelAperture)
{
    return IODeviceMemory::withRange((mach_vm_address_t)fFbPhys, (IOByteCount)fPitch * fHeight);
}

IODeviceMemory *IOIntelFramebuffer::getVRAMRange(void)
{
    return getApertureRange(kIOFBSystemAperture);
}

IOReturn
IOIntelFramebuffer::getInformationForDisplayMode(IODisplayModeID, IODisplayModeInformation *info)
{
    bzero(info, sizeof(*info));
...
UInt64
IOIntelFramebuffer::getPixelFormatsForDisplayMode(IODisplayModeID, IOIndex)
{
    return (UInt64)(uintptr_t)getPixelFormats();
}

IOReturn
IOIntelFramebuffer::getPixelInformation(IODisplayModeID displayMode, IOIndex depth,
...
IOItemCount IOIntelFramebuffer::getDisplayModeCount(void) { return 1; }

IOReturn IOIntelFramebuffer::getDisplayModes(IODisplayModeID *allDisplayModes)
{
