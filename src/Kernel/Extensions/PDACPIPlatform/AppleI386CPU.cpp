...
IOCPUInterruptController *AppleI386CPU::sCpuIC = NULL;

IOService *AppleI386CPU::probe(IOService *provider, SInt32 *score) {
	return this;
}

bool AppleI386CPU::startCommon(unsigned numCPUs) {
	if (startCommonCompleted) return true;
	if (numCPUs < 1) numCPUs = 1;

	if (sCpuIC == 0) {
		sCpuIC = new AppleI386CPUInterruptController;
		if (sCpuIC == 0) return false;
		if (sCpuIC->initCPUInterruptController((int)numCPUs) != kIOReturnSuccess)
			return false;

		sCpuIC->attach(this);
		sCpuIC->registerCPUInterruptController();
	}
...
bool AppleI386CPU::prepareSecondary(UInt32 cpuNumber) {
