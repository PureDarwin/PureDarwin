...
/* HACK here to forward declare this md_* routine, only in the ARM assembler */
...
static arm_feature_set cpu_variant /* HACK */ = ARM_FEATURE (ARM_AEXT_V4T, FPU_FPA);
  ...
  do
      ...
      if (*str == '-') /* We have the start of a range expression */
	{
	  ...
	  if ((high_range = arm_typed_reg_parse (&str, regtype, NULL, NULL))
              == FAIL)
	    {
	      inst.error = /* HACK gettext */ (reg_expected_msgs[regtype]);
	      return FAIL;
	    }
...
#endif
	symbolP = symbol_new (name, undefined_section, 0, 0, 0, &zero_address_frag);
#ifdef OBJ_COFF
...
#endif
#ifdef OBJ_ELF
/* unused OBJ_ELF directives removed */
#else
  { "word",	   cons, 4},
