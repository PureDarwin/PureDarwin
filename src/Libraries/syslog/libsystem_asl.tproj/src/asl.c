 ...
 * preferences. With no such preference store here, "the caller asked for
 * tracing, so let it trace" is the honest answer. This mirrors what ravynOS
 * does in its own syslog.c for the identical collision.
 */
#define os_log_shim_enabled(addr) _pd_asl_os_log_shim_enabled(addr)
static bool
_pd_asl_os_log_shim_enabled(void *addr)
{
	(void)addr;
	return true;
}
