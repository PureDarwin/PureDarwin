...
// include the below to replace the above
...
// define this when including socket.h to get at SO_EXECPATH
#define PRIVATE 1
...
// this is part of IOKitUser...
//#include <bootfiles.h>
// ...but is only needed to do this
...
WriteMyPropertyListToFile(CFPropertyListRef plist, const char *posixfile)
{
    ...
    // _sjc_ this was left here with editor placeholders! no idea what's meant to be going on here!
//    resourceData = CFPropertyListCreateWithData(NULL, CFDataRef data, CFOptionFlags options, CFPropertyListFormat *format, CFErrorRef *error);
    
    // TODO: check if format is correct
    resourceData = CFPropertyListCreateData(NULL, plist, kCFPropertyListXMLFormat_v1_0, 0, &error);
    
	if (resourceData == NULL) {
        launchctl_log(LOG_ERR, "%s: CFPropertyListCreateXMLData(%s) failed: %d", getprogname(), posixfile, CFErrorGetCode(error));
	}
...
system_specific_bootstrap(bool sflag)
{
	...
	if ((lda = read_plist_file(AUDITD_PLIST_FILE, false, false)) != NULL && ((ldb = launch_data_dict_lookup(lda, LAUNCH_JOBKEY_DISABLED)) == NULL || job_disabled_logic(ldb) == false)) {
        // _sjc_ commented out because audit_quick_start() isn't in libbsm nowadays
...
//    NSSearchPathEnumerationState es = 0;
    CFSearchPathEnumerationState es = 0;
	...
	while ((ch = getopt(argc, argv, "wFS:D:")) != -1) {
		...
		case 'D':
			if (strcasecmp(optarg, "all") == 0) {
                es |= kCFAllDomainsMask; // was NSUserDomainMask
			} else if (strcasecmp(optarg, "user") == 0) {
                es |= kCFUserDomainMask; // was NSUserDomainMask;
			} else if (strcasecmp(optarg, "local") == 0) {
                es |= kCFLocalDomainMask; // NSLocalDomainMask;
			} else if (strcasecmp(optarg, "network") == 0) {
                es |= kCFNetworkDomainMask; // NSNetworkDomainMask;
			} else if (strcasecmp(optarg, "system") == 0) {
                es |= kCFSystemDomainMask; // NSSystemDomainMask;
			} else {
				badopts = true;
			}
			break;
...
kill_cmd(int argc, char *const argv[]) {
	...
	if (signo == 0) {
#define IF_SIGNAL_NAME(name) if (strcasecmp(argv[1], #name) == 0 || strcasecmp(argv[1], "SIG" #name) == 0) signo = SIG##name
		IF_SIGNAL_NAME(HUP);
