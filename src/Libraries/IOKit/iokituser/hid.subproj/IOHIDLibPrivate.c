...
/*
 * Needs the whole IOHIDEvent API, which the drop does not ship (IOHIDEvent.c is
 * a license header only). Gated until the event system exists; the body is
 * unreachable without it anyway, since only an event system sets
 * kIOHIDDebugPerfEvent.
 */
#if PD_HAVE_IOHID_EVENT_SYSTEM
...
#else
void _IOHIDDebugEventAddPerfData(IOHIDEventRef event __unused,
                                 int timepoint __unused,
                                 uint64_t timestamp __unused) { }
#endif /* PD_HAVE_IOHID_EVENT_SYSTEM */
