...
/*
 * PureDarwin reconstruction: used throughout hid.subproj but stripped from
 * this header along with IOHIDManagerPersistentProperties.h. Guarded release,
 * clearing the variable so a second pass cannot double-release.
 */
/* atomic_fetch_or on the dispatchStateMask ivars, and MIN()/min(); the drop
 * relies on both arriving transitively. */
#include <stdatomic.h>
#include <sys/param.h>
/* nil, used by the IOHID_DYN_LINK_DYLIB bodies at the end of this header. */
#include <objc/objc.h>

#ifndef CFRELEASE_IF_NOT_NULL
#define CFRELEASE_IF_NOT_NULL(x) do { if (x) { CFRelease(x); (x) = NULL; } } while (0)
#endif
