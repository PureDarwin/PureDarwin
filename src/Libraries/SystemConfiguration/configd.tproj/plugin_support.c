...
/*
 * Each entry below is compiled in only when its plugin is part of the build,
 * because the table takes the address of the plugin's load/prime function and
 * would otherwise leave configd with undefined symbols. The plugins that are
 * built define HAVE_PLUGIN_<name>; the rest are simply absent, and a bundle
 * marked "builtin" in its Info.plist with no matching entry is skipped by the
...
#endif
#ifdef	HAVE_PLUGIN_InterfaceNamer
extern SCDynamicStoreBundleLoadFunction		load_InterfaceNamer;
#endif
#ifdef	HAVE_PLUGIN_KernelEventMonitor
...
#endif
#ifdef	HAVE_PLUGIN_LinkConfiguration
extern SCDynamicStoreBundleLoadFunction		load_LinkConfiguration;
#endif
#ifdef	HAVE_PLUGIN_PreferencesMonitor
