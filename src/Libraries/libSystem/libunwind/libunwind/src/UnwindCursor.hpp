...
private:
  ...
  int stepWithCompactEncoding(Registers_arm64 &) {
    return CompactUnwinder_arm64<A>::stepWithCompactEncoding(
        _info.format, _info.start_ip, _addressSpace, _info.flags, _registers);
  }
#endif
  ...
  bool compactSaysUseDwarf(Registers_arm &, uint32_t *offset) const {
    if ((_info.format & UNWIND_ARM_MODE_MASK) == UNWIND_ARM_MODE_DWARF) {
      if (offset)
        *offset = (_info.format & UNWIND_ARM_DWARF_SECTION_OFFSET);
      return true;
    }
    return false;
