...
/* Handle returned by mDNSPlatformUDPSocket() for a unicast question's
 * LocalSocket. "shared" marks the descriptor as owned by mDNSPlatformInit()
 * (m->p->unicastSocket4), so mDNSPlatformUDPClose() must not close it. */
struct UDPSocket_struct
{
    // port MUST BE FIRST: mDNSCoreReceive and ExpectingUnicastResponseForRecord
    // read it through their own local declaration of this struct (see
    // mDNS.c/DNSCommon.c), which contains only this leading field.
    mDNSIPPort port;
    int        fd;
    mDNSBool   shared;      // fd is owned by mDNSPlatformInit(); do not close
};
