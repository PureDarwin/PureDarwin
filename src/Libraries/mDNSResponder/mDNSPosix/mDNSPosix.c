                                       ...
                                       mDNSIPPort dstPort, mDNSBool useBackgroundTrafficClass)
{
    ...
    if (dst->type == mDNSAddrType_IPv4)
    {
        ...
        // A unicast question supplies its own socket (mDNSPlatformUDPSocket).
        if (!thisIntf && src && src->fd >= 0) sendingsocket = src->fd;
    }
    ...
    /* Both failure paths below are silent by default: a negative sendingsocket
     * skips the send entirely while leaving err at 0 (reported as success), and
     * the four "transient" errnos return before reaching any LogMsg. Neither
     * shows up as a packet on the wire either, so say so explicitly. */
    if (sendingsocket < 0)
        LogInfo("mDNSPlatformSendUDP: no socket for %#a (thisIntf %p) - packet dropped", dst, thisIntf);

    if (sendingsocket >= 0)
    {
        err = sendto(sendingsocket, msg, (char*)end - (char*)msg, 0, (struct sockaddr *)&to, GET_SA_LEN(to));
        if (err < 0)
            LogInfo("mDNSPlatformSendUDP: sendto socket %d -> %#a:%d failed errno %d (%s)",
                    sendingsocket, dst, mDNSVal16(dstPort), errno, strerror(errno));
        else
            LogInfo("mDNSPlatformSendUDP: sendto socket %d -> %#a:%d sent %d bytes",
                    sendingsocket, dst, mDNSVal16(dstPort), err);
    }
...
mDNSlocal void SocketDataReady(mDNS *const m, PosixNetworkInterface *intf, int skt)
{
    ...
    if (packetLen >= 0)
    {
        /* The destination port was previously hardcoded to MulticastDNSPort.
         * That is correct for the per-interface multicast sockets, which are
         * bound to 5353, but wrong for the unicast socket: the core matches a
         * unicast DNS response to its question by comparing this port against
         * the question's LocalSocket->port (see
         * ExpectingUnicastResponseForRecord), so a reply to an ephemeral port
         * never matched and every unicast query timed out. */
        mDNSIPPort destPort = MulticastDNSPort;
        if (!intf)
        {
            struct sockaddr_storage ss;
            socklen_t sslen = sizeof(ss);
            if (getsockname(skt, (struct sockaddr *)&ss, &sslen) == 0)
            {
                if (ss.ss_family == AF_INET)
                    destPort.NotAnInteger = ((struct sockaddr_in *)&ss)->sin_port;
#if HAVE_IPV6
                else if (ss.ss_family == AF_INET6)
                    destPort.NotAnInteger = ((struct sockaddr_in6 *)&ss)->sin6_port;
#endif
            }
        }

        LogInfo("SocketDataReady: skt %d intf %p len %d from %#a:%d to %#a:%d",
                skt, intf, packetLen, &senderAddr, mDNSVal16(senderPort),
                &destAddr, mDNSVal16(destPort));
        mDNSCoreReceive(m, &packet, (mDNSu8 *)&packet + packetLen,
                        &senderAddr, senderPort, &destAddr, destPort, InterfaceID);
    }
}
...
/* The unicast-query socket.
 *
 * This was previously an unimplemented stub returning NULL, which silently
 * disabled all unicast DNS: uDNS_CheckCurrentQuestion() calls this to obtain
 * q->LocalSocket and, on NULL, sets mStatus_NoMemoryErr and skips
 * mDNSSendDNSMessage() entirely. No query was ever transmitted, yet the core
 * still counted the question as sent-and-unanswered, so the failure looked
 * like an unresponsive DNS server rather than a missing platform function.
 *
 * The socket wraps m->p->unicastSocket4 rather than creating a new descriptor.
 * That socket is already created in mDNSPlatformInit(), already added to the
 * event loop's fd set, and already dispatched through SocketDataReady() to
 * mDNSCoreReceive() - which is what delivers the reply. A fresh descriptor per
 * question would need all of that wiring duplicated to be readable at all.
 * Sharing one source port across outstanding queries is what a conventional
 * stub resolver does; responses are demultiplexed by the DNS message ID, which
 * the core already tracks per question as TargetQID.
 */
// Set by mDNSPlatformInit(); mDNSPlatformUDPSocket() has no mDNS parameter and
// mDNSStorage is defined by the client program, not by this platform layer.
mDNSlocal mDNS *gPosixMDNS = mDNSNULL;

mDNSexport UDPSocket *mDNSPlatformUDPSocket(mDNSIPPort port)
{
    UDPSocket *sock;

    (void)port;     // Always the ephemeral port of the shared unicast socket

    if (gPosixMDNS == mDNSNULL || gPosixMDNS->p == NULL || gPosixMDNS->p->unicastSocket4 < 0)
    {
        LogMsg("mDNSPlatformUDPSocket: no unicast socket available");
        return NULL;
    }

    sock = (UDPSocket *) mDNSPlatformMemAllocateClear(sizeof(*sock));
    if (sock == NULL) return NULL;

    sock->fd = gPosixMDNS->p->unicastSocket4;
    sock->shared = mDNStrue;

    /* The core matches a unicast response to its question by comparing this
     * port against the packet's destination port, so it has to be the real
     * bound port rather than whatever the caller asked for. */
    {
        struct sockaddr_storage ss;
        socklen_t sslen = sizeof(ss);
        if (getsockname(sock->fd, (struct sockaddr *)&ss, &sslen) == 0 &&
            ss.ss_family == AF_INET)
        {
            sock->port.NotAnInteger = ((struct sockaddr_in *)&ss)->sin_port;
        }
        else
        {
            LogMsg("mDNSPlatformUDPSocket: getsockname failed on fd %d", sock->fd);
            mDNSPlatformMemFree(sock);
            return NULL;
        }
    }
    return sock;
...
{
    if (sock == NULL) return;
    // Never close the shared descriptor - it belongs to mDNSPlatformInit().
    if (!sock->shared && sock->fd >= 0) close(sock->fd);
    mDNSPlatformMemFree(sock);
}
    ...
    DNameListElem **BrowseDomains, mDNSBool ackConfig)
{
     ...
     *
     * uDNS_SetupDNSConfig() marks every entry in m->DNSServers for deletion,
     * calls this, and only restores them when this returns false - a true
     * return means "the configuration changed and I have supplied the new
     * servers". This platform supplies none: on Posix the server list comes
     * from ParseDNSServers() reading /etc/resolv.conf, and setservers is
     * ignored here entirely. Returning true therefore claimed a change that
     * never happened and left every server flagged DNSServerFlag_Delete, so
     * SetValidDNSServers() skipped them all and no unicast question ever had a
     * server to send to (validDNSServers == 0, zero packets sent).
     */
    return mDNSfalse;
}
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
{
    ...
    gPosixMDNS = m;     // for mDNSPlatformUDPSocket(), which takes no mDNS arg
