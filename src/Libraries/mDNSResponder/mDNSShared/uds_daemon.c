...
mDNSexport void udsserver_info_dump_to_fd(int fd)
{
    ...
    if (!m->DNSServers) LogToFD(fd, "<None>");
    else
    {
        DNSServer *sp;
        for (sp = m->DNSServers; sp; sp = sp->next)
        {
            char *ifname = InterfaceNameForID(m, sp->interface);
            LogToFD(fd, "%#a:%d domain %##s scope %d flags 0x%x if %s",
                    &sp->addr, mDNSVal16(sp->port), sp->domain.c,
                    (int)sp->scopeType, sp->flags, ifname ? ifname : "any");
        }
    }
