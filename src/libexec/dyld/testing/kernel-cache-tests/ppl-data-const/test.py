...
# PPL data const should be folded in under data const and allowed to have fixups

def check(kernel_cache):
    kernel_cache.buildKernelCollection("arm64", "/ppl-data-const/main.kc", "/ppl-data-const/main.kernel", "/ppl-data-const/extensions", ["com.apple.foo", "com.apple.bar"], [])
    kernel_cache.analyze("/ppl-data-const/main.kc", ["-layout", "-arch", "arm64"])
    ...
    assert kernel_cache.dictionary()["dylibs"][0]["segments"][3]["name"] == "__PPLDATA_CONST"
    ...
    kernel_cache.analyze("/ppl-data-const/main.kc", ["-symbols", "-arch", "arm64"])
    # main.kernel
    # int g = &x;
    ...
    kernel_cache.analyze("/ppl-data-const/main.kc", ["-fixups", "-arch", "arm64"])
    # main.kernel
    # int g = &x;
...
# [~]> xcrun -sdk iphoneos.internal cc -arch arm64 -Wl,-static -mkernel -nostdlib -Wl,-add_split_seg_info -Wl,-rename_section,__TEXT,__text,__TEXT_EXEC,__text -Wl,-e,__start -Wl,-pagezero_size,0x0 -Wl,-pie -Wl,-rename_section,__DATA,__data,__PPLDATA_CONST,__data -Wl,-segprot,__PPLDATA_CONST,r--,r-- main.c -o main.kernel
