...
&Usage() if $opt_h;
$COUNT = 1 if $opt_c;
$FILTER = "&& (execname == strstr(execname,\"$opt_n\")) || (\"$opt_n\" == strstr(\"$opt_n\",execname))" if defined $opt_n;
$FILTER = "&& pid == $opt_p" if defined $opt_p;

#
# Find the MacOSX SDK path
#
$sdk_path = `xcrun --sdk macosx --show-sdk-path` or die "ERROR: trying to find the MacOSX SDK path: $?\n";
chomp($sdk_path);

#
#  Load errno descriptions
#
open(ERRNO, "$sdk_path/usr/include/sys/errno.h") || die "ERROR1: reading errno.h: $!\n";
