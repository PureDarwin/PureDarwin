...
static ls_event_info_t g_event_info[LS_MAX_EVENTS] = {
	...
	{ 'I',	"CPU", 	"Profiling interrupt",			"nsec",
...
static void
lockstat_mergesort(int (*cmp)(lsrec_t *, lsrec_t *), lsrec_t **a, lsrec_t **b, int n)
{
	int m = n / 2;
	int i, j;

	if (m > 1)
		lockstat_mergesort(cmp, a, b, m);
	if (n - m > 1)
		lockstat_mergesort(cmp, a + m, b + m, n - m);
	for (i = m; i > 0; i--)
		b[i - 1] = a[i - 1];
	for (j = m - 1; j < n - 1; j++)
		b[n + m - j - 2] = a[j + 1];
...
status_init(void)
{
	...
	/*
	 * APPLE NOTE:
	 * The timer sends a SIGALRM signal to the process when it expires.
	 */
	...
	tv.it_value.tv_sec = val / MICROSEC;
	tv.it_value.tv_usec = val % MICROSEC;
	tv.it_interval = tv.it_value;

	if (setitimer(ITIMER_REAL, &tv, NULL) != 0)
		dfail("cannot create the ITIMER_REAL timer");
}
...
main(int argc, char **argv)
{
	...
	do {
		status_check();
		err = waitpid(child, &status, WNOHANG);
	} while (err <= 0);
	...
	err = posix_memalign((void**) &data_buf, sizeof(uint64_t),
	                     (g_nrecs + 1) * g_recsize);
	if (err != 0)
	...
	if (g_gflag) {
		...
		err = posix_memalign((void**) &newlsp, sizeof(uint64_t),
	 ...
	 */
	if (g_gflag) {
		lockstat_mergesort(lockcmp, sort_buf, merge_buf, g_nrecs_used);
	 ...
	 */
	if (g_wflag) {
		lockstat_mergesort(lock_and_count_cmp_anywhere,
	 ...
	 */
	if (g_Wflag) {
		lockstat_mergesort(site_and_count_cmp_anylock,
		    sort_buf, merge_buf, g_nrecs_used);
	...
	if (g_Pflag)
		lockstat_mergesort(timecmp, sort_buf, merge_buf, g_nrecs_used);
	else
		lockstat_mergesort(countcmp, sort_buf, merge_buf, g_nrecs_used);
