...
#endif
{
  ...
  int handler_switch_value;
  void* thrown_ptr = (ue_header->exception_class & 1) ?
          ((__cxa_dependent_exception*)(ue_header + 1) - 1)->primaryException :
          ue_header + 1;
  ...
  try 
  ...
  catch(...) 
    {
      ...
      __cxa_exception *new_xh = globals->caughtExceptions;
        if (new_xh->unwindHeader.exception_class & 1)
        {
            __cxa_dependent_exception* deh =
                reinterpret_cast<__cxa_dependent_exception*>(new_xh + 1) - 1;
            new_xh = static_cast<__cxa_exception*>(deh->primaryException) - 1;
        }
	...
	throw;
      ...
      // We get here if the compiler called __cxa_call_unexpected()
      // because some thrown exception did not match the exception
      // specification.  Plus when the unexpected handler was called,
      // it threw an exception, and that exception also does not match
      // the specification allowed.  
      // C++ spec 15.5.2 says that if the specification allows bad_exception
      // we should throw that, otherwise terminate.
	...
	if (check_exception_spec(&info, bad_excpt, 0, xh_switch_value)) { 
	  typedef void (*proc_t)();
	  proc_t thrower = (proc_t)dlsym(RTLD_DEFAULT, "_ZSt21__throw_bad_exceptionv");
	  if ( thrower != NULL ) {
	    (*thrower)();
	  }
	}
