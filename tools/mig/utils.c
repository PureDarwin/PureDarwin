...
WriteIdentificationString(FILE *file)
{
  ...
  fprintf(file, " * stub generated by " MIG_VERSION "\n");
...
GetArgStar(argument_t *arg, u_int mask) {
  if (arg->argType->itNativePointer)
    return "*";
  if ((mask & akbUserArg) == akbUserArg && arg->argByReferenceUser)
    return "*";
  if ((mask & akbServerArg) == akbServerArg && arg->argByReferenceServer)
    return "*";
  return "";
}

static void
WriteVarDecl(FILE *file, argument_t *arg, const char *argType, u_int mask)
{
  char *cnst = ((arg->argFlags & flConst) &&
                (IS_VARIABLE_SIZED_UNTYPED(arg->argType) ||
                 arg->argType->itNoOptArray || arg->argType->itString)) ? "const " : "";
    fprintf(file, "\t%s%s ", cnst, argType);
  
  if (EmitCountAnnotations && arg->argType->itVarArray) {
    /*
     * We do not print __counted_by attributes on fixed-size arrays. Fixed-sizd
     * arrays have no reason to need it, and "variably sized" arrays as function
     * parameters actually already come with a minimum length that the compiler
     * will enforce exactly as if there was a __counted_by attribute.
     */
    argument_t *countArg = NULL;
    if (arg->argCount) {
      countArg = arg->argCount;
    } else if (arg->argSameCount) {
      countArg = arg->argSameCount->argCount;
    } else if (arg->argCInOut) {
      countArg = arg->argCInOut;
    }
    if (countArg && akCheckAll(countArg->argKind, mask)) {
      fprintf(file, "__counted_by(%s%s) ", GetArgStar(countArg, mask), countArg->argVarName);
    }
  }

  fprintf(file, "%s%s", GetArgStar(arg, mask), arg->argVarName);
}

void
WriteUserVarDecl(FILE *file, argument_t *arg)
{
  WriteVarDecl(file, arg, arg->argType->itUserType, akbUserArg);
}

void
WriteServerVarDecl(FILE *file, argument_t *arg)
{
  WriteVarDecl(file, arg, arg->argType->itTransType, akbServerArg);
}
