...
let
  ...
  deps = [
    ...
    xorgproto libpng
    gtk3 libXres startupNotification
  ];
...
stdenv.mkDerivation {
  ...
  postPatch = ''
    ...
    # --version-script is GNU ld; Mach-O ld64 rejects it outright ("unknown
    # option"). Its Mach-O counterpart is -exported_symbols_list with a different
    # file format, and the map only restricts an already-correct export set, so
    # dropping it costs nothing here.
    substituteInPlace libwnck/meson.build \
      --replace "  '-Wl,--version-script,@0@/@1@'.format(meson.current_source_dir(), mapfile)," ""
  '';
...
[built-in options]
c_args = ['-isysroot', '$DARWIN_SDK_ROOT', '-mmacosx-version-min=26.5', '-Qunused-arguments', '-U_FORTIFY_SOURCE', '-D_FORTIFY_SOURCE=0', '-fno-stack-protector', '-DCAIRO_HAS_GOBJECT_FUNCTIONS=1', '-Ddngettext(Domain,Singular,Plural,N)=((N)==1?(Singular):(Plural))', '-I${libSystem}/usr/include', ${lib.concatMapStringsSep ", " (dep: "'-I${lib.getDev dep}/include'") deps}]
